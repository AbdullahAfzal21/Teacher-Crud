...
app.post("/login", async (req, res) => {
    ...
    try {
        ...
        const token = jwt.sign({ email: user.email,password:password,id:user._id }, mysecretkey);
        res.status(200).json({
...
app.post("/create-student", check, async (req, res) => {
  ...
  try {
      ...
      transporter.sendMail(mailOptions, (error, info) => {
          if (error) {
              ...
              return res.status(500).send("Error creating student and sending email: " + error.message);
          }
...
app.post("/create-course", check, async (req, res) => {
    const { name, author } = req.body;
    const studentId = req.user.id;
    try {
        const teacher = await Student.findById(studentId);
        if (!teacher) {
            return res.status(404).send("student not found");
        }
...
app.post("/course/:courseId/update", check, async (req, res) => {
  ...
  try {
      ...
      if (course.studentId != studentId) {
          return res.status(403).send("You do not have permission to update this course");
      }
      ...
      res.status(200).json({ message: "Course updated successfully", student: updatedCourse });
  } catch (error) {
      console.error("Error updating Course:", error);
      res.status(500).send("Error updating Course: " + error.message);
  }
});
app.post("/student/:studentId/update", check, async (req, res) => {
    ...
    try {
        ...
        if (student.teacher !=teacherId) {
            return res.status(403).send("You do not have permission to update this student");
        }
...
app.delete("/course/:courseId/delete", check, async (req, res) => {
  const { courseId } = req.params;
...
app.delete("/student/:studentId/delete", check, async (req, res) => {
    const { studentId } = req.params;
    ...
    try {
        ...
        if (student.teacher.toString() !== teacherId) {
            return res.status(403).send("You do not have permission to delete this student");
        }
...
app.get("/teacher/:teacherId/students", check, async (req, res) => {
